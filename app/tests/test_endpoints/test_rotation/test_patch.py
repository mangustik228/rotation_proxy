...
async def test_change_proxy_base(insert_5_proxy_to_change, async_client: AsyncClient):
...
async def test_change_proxy_error_db(insert_5_proxy_to_change, async_client: AsyncClient):
    builder = ProxyBuilderPatch()
    builder.set_proxy_id(2)
    data = builder.build()
    await async_client.patch("/proxies/rotations", json=data)
    result = await R.Error.count_items()
    assert result == 1


async def test_change_proxy_not_available(insert_5_proxy_to_change, async_client: AsyncClient):
    builder = ProxyBuilderPatch()
    builder.set_proxy_id(1)
    for _ in range(4):
        data = builder.build()
...
async def test_change_proxy_expired_from_another_service(insert_5_proxy_to_change, async_client: AsyncClient):
    builder = ProxyBuilderPatch()
    for i in range(1, 6):
